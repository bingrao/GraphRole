...
if __name__ == "__main__":
    ...
    df_broadcast = spark.sparkContext.broadcast(df_roles.collect())  # Array of Row[model, date, roles]
    ...
    def getdatesfromlist(model, data_list):
        reglist = list(filter(lambda rowdata: rowdata.__getitem__('model') == model, data_list))
        dates = set(map(lambda rowdata: rowdata.__getitem__('date'), reglist))
        return dates

    @udf(returnType=StringType())
    def gettransition(model, date):
        dates = getdatesfromlist('base', df_broadcast.value)
        reg = pd.DataFrame()
        if date - timedelta(windows - 1) in dates:  # make sure include enought data to get transition
            if model == "base":
                for k in range(windows):
                    ...
                    role_str = getrolestrfromlist(model, iterdate, df_broadcast.value)
                ...
                return getrolestrfromlist(model, date, df_broadcast.value)
        else:
            return None


    @udf(returnType=StringType())
    def gettransitiondebug(model, date):  # debug and test
        gs = getrolestrfromlist(model, date, df_broadcast.value)
        if model == "base":
    ...
    def getfrobeniousloss(model, date, pred):
        ...
        if pre_date in dates:
            base_str = getrolestrfromlist('base', pre_date, df_broadcast.value)
            if base_str is None:
                return None
            else:
